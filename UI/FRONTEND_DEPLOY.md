...
**중요**: 
- 빌드 명령어가 정확히 `npm install && npm run build`인지 확인
- Publish Directory가 정확히 `dist` (소문자)인지 확인
- 빌드가 완료되어야 `dist` 폴더가 생성됩니다
...
### 빌드 오류 - "Publish directory dist does not exist!"

**증상**: 빌드가 실패하거나 `dist` 폴더가 생성되지 않음

**원인**:
1. 빌드 명령어가 실패했지만 오류가 명확하지 않음
...
1. **빌드 로그 확인**
   ...
   - `npm run build`가 성공했는지 확인

2. **Root Directory 확인**
   - **Settings** → **Root Directory**: `UI` (대문자)
   - 저장 후 재배포

3. **빌드 명령어 확인**
   - **Settings** → **Build Command**: `npm install && npm run build`
   - 정확히 위와 같이 입력 (공백 포함)
   - 또는 더 안정적인 방법: `npm ci && npm run build`

4. **Publish Directory 확인**
   - **Settings** → **Publish Directory**: `dist` (소문자)
   - 저장 후 재배포
...
6. **빌드 명령어 개선** (필요 시)
   ...
   ```
   - `npm ci`는 `package-lock.json`을 사용하여 더 안정적인 빌드
